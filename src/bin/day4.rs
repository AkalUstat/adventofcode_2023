...
fn algo() -> (usize, usize) {
    let file_r: BufReader<_> = file_reader("./aoc-inputs/2023/day4.txt");
    ...
    for (indx, ln) in lines.iter().enumerate() {
        ...
        let matches_sum =
            total_matches.fold(0, |acc, _num| if acc < 1 { acc + 1 } else { acc * 2 });
    ...
    for i in 1..=total_cards {
        ...
        for j in (i + 1)..=(i + card.borrow().num_matches) {
            ...
            (*future_card)
                .borrow_mut()
                .increment(card.borrow().count * 1);
        }
    }
    many_scratches_collector += cards
        .iter()
